...
// Get logged-in student's coordinates
$studentName = $_SESSION['name'];
$studentQuery = mysqli_query($connect, "SELECT latitude, longitude FROM student WHERE username='$studentName'");
$studentData = mysqli_fetch_assoc($studentQuery);

$studentLat = $studentData['latitude'];
$studentLng = $studentData['longitude'];
$radius = 10; 

// Haversine SQL Query to find tutors within 10km
$query = "
    SELECT *, (
        6371 * acos(
...
$stmt->bind_param("dddd", $studentLat, $studentLng, $studentLat, $radius);
$stmt->execute();
...
<h2><i class="fas fa-book"></i> Tutors Available in "<?= htmlspecialchars($radius); ?>KM Radius"</h2>
...
<?php if ($result && $result->num_rows > 0): ?>
    ...
    ?>
      <div class="tutor-card row align-items-center <?= $isTrusted ? 'trusted-card' : '' ?>">
        ...
        <div class="col-md-3 btn-set">
            <button type="button" class="btn btn-custom request-btn"
                onclick="openSubjectPopup(
                    '<?= $tid ?>',
                    '<?= htmlspecialchars($row['name'], ENT_QUOTES) ?>',
                    '<?= htmlspecialchars($row['email'], ENT_QUOTES) ?>',
                    '<?= htmlspecialchars($row['subject1'], ENT_QUOTES) ?>',
                    '<?= htmlspecialchars($row['subject2'], ENT_QUOTES) ?>',
                    '<?= htmlspecialchars($row['subject3'], ENT_QUOTES) ?>'
                )">Apply Now</button>
    ...
    <?php endwhile; ?>
<?php else: ?>
    <div class="alert alert-warning text-center">No tutors found in "<?= htmlspecialchars($radius); ?>KM Radius"</div>
<?php endif; ?>
</div>
...
<!-- JS -->
<script>
function openSubjectPopup(tid, name, email, subject1, subject2, subject3) {
