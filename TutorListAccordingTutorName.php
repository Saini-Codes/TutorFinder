...
if (empty($nameInput)) {
    echo '
    <div style="max-width: 500px; margin: 60px auto; background: #fff3cd; color: #856404; border: 1px solid #ffeeba; border-radius: 12px; padding: 20px 30px; font-family: Poppins, sans-serif; box-shadow: 0 5px 15px rgba(0,0,0,0.1); text-align: center;">
        <h4 style="margin-bottom: 10px;">⚠ Missing Tutor Name</h4>
        <p>Please enter a tutor name to continue your search.</p>
        <a href="SearchTutorByName.php" style="display: inline-block; margin-top: 12px; padding: 10px 20px; background: #ffc107; color: #212529; text-decoration: none; border-radius: 8px; font-weight: 600;">🔙 Go Back</a>
    </div>';
    exit;
}

$sql = "SELECT * FROM tutor WHERE LOWER(name) LIKE LOWER(?)";
...
<?php if ($result && $result->num_rows > 0): ?>
    ...
    <div class="tutor-card row align-items-center <?= $isTrusted ? 'trusted-card' : '' ?>">
        <div class="col-md-3 text-center">
            <img src="<?= $logoPath ?>" class="tutor-logo" onclick="openModal(this.src)">
        </div>
        <div class="col-md-6">
            <div class="tutor-details">
                <h4><?= htmlspecialchars($row['name']) ?> <span style="font-size:14px;">(<?= htmlspecialchars($row['gender']) ?>)</span>
                <?php if ($isTrusted): ?>
                    <span class="trusted-badge" title="TutorFinder Trusted!!"><img src="Images/TrustedBadge.png" alt="Trusted" class="trusted-icon"></span>
                <?php endif; ?>
                </h4>
                <div class="rating-display">
                    ⭐ Rating: <span><?= $stars ?></span> <?= is_numeric($avgRating) ? "($avgRating)" : "" ?>
                    <button type="button" class="btn btn-custom btn-rate" onclick="openRatingModal('<?= $tid ?>')">Rate</button>
                </div>
                <div>
        ...
        <div class="col-md-3">
            <button class="btn btn-custom request-btn" onclick="openSubjectPopup('<?= $tid ?>','<?= htmlspecialchars($row['name']) ?>','<?= htmlspecialchars($row['email']) ?>','<?= htmlspecialchars($row['subject1']) ?>','<?= htmlspecialchars($row['subject2']) ?>','<?= htmlspecialchars($row['subject3']) ?>')">Apply Now</button>
            <button class="btn btn-custom btn-reviews" onclick="toggleReviews('rev<?= $tid ?>', this)">View Reviews</button>
        </div>
        <div class="col-12">
    ...
    <?php endwhile; ?>
<?php else: ?>
    <div class="alert alert-warning text-center">No tutors found named "<?= htmlspecialchars($nameInput); ?>"</div>
<?php endif; ?>
</div>

<!-- SUBJECT POPUP MODAL -->
<div class="modal" id="subjectModal">
    <div style="background:#fff; padding:30px; border-radius:15px; width:90%; max-width:400px; position:relative;">
        <button onclick="closeSubjectModal()" style="position:absolute; top:10px; right:15px; background:none; border:none; font-size:24px;">&times;</button>
        <h4 style="text-align:center;">Select a Subject</h4>
        <form action="TutorListAccordingLocationSubmit.php" method="POST" onsubmit="return validateSubjectSelect(this)">
            <input type="hidden" name="tutor_id" id="popup_tid">
            <input type="hidden" name="name" id="popup_name">
            <input type="hidden" name="email" id="popup_email">
            <select name="subject" id="popup_subject" class="form-select" required>
...
function toggleReviews(id, btn) {
    ...
    btn.textContent = elem.style.display === "block" ? "Close Reviews" : "View Reviews";
}

function openSubjectPopup(tid, name, email, s1, s2, s3) {
    document.getElementById('popup_tid').value = tid;
    document.getElementById('popup_name').value = name;
    document.getElementById('popup_email').value = email;
    const select = document.getElementById('popup_subject');
    select.innerHTML = '<option value="" disabled selected>-- Choose Subject --</option>';
    [s1, s2, s3].forEach(sub => {
        if (sub && sub.trim()) {
...
function validateSubjectSelect(form) {
    const subject = form.querySelector('#popup_subject').value;
    if (!subject) {
