...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($student_email) || empty($reason) || empty($mail_subject)) {
        echo 'Error: Missing required fields.';
        exit;
    }

    $stmt = $connect->prepare("DELETE FROM student WHERE email = ?");
    ...
    echo "
        ...
        '>
            <h2 style='color: #0ef05c; margin-bottom: 20px;'>✅ Student Removed Successfully</h2>
            <p style='font-size: 18px;'>An email notification has been sent to: <strong style='color: #0ef0e5;'>$student_email</strong></p>
            <br>
            <a href='StudentList.php' style='
        ...
        echo "<div style='color: #ff4b5c; font-weight: bold; text-align: center; margin-top: 50px;'>❌ Failed to remove student.</div>";
    }
} else {
